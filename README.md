...
> 시스템 실험 자동화를 위한 Opencode orchestrator와 세 hidden subagent. **계획 ≠ 검토 ≠ 실행**, **모든 실행은 스크립트 파일을 거침**, **실패 시 즉시 중단** 원칙으로 재현성과 결과 무결성을 보장합니다.
...
하나의 사용자-facing primary agent와 세 hidden subagent를 제공합니다.

- **experiment-orchestrator** — 사용자의 유일한 Tab 선택 entrypoint입니다. 대화, 승인 gate, planner/reviewer/executor 호출 순서를 조율합니다.
...
- **experiment-reviewer** — hidden subagent입니다. draft plan과 generated scripts를 read-only 검토하고 `PASS`, `PASS_WITH_WARNINGS`, `BLOCKED` review artifact를 기록합니다. plan이나 scripts는 수정하지 않습니다.
...
1. **계획 ≠ 검토 ≠ 실행** — orchestrator가 planner, reviewer, executor를 분리 호출. 승인 후 동결된 `experiments/<id>/plan.yaml`이 실행 계약.
2. **모든 실행은 스크립트 파일을 거침** — bash 직접 실행 금지. 히스토리가 항상 파일로 남음.
3. **실패 시 즉시 중단** — 자동 재시도/fallback/cleanup 없음. 부분 결과 보존.
4. **두 단계 사용자 승인 + 두 reviewer gate** — ① draft plan reviewer gate ② ID 기반 draft plan 승인 ③ script reviewer gate ④ 스크립트 실행 직전 재확인.
...
│   ├── experiment-reviewer.md           # hidden subagent (계획/스크립트 검토)
│   └── experiment-executor.md           # hidden subagent (실행)
├── context/
│   └── experiment-agents-context.md     # 네 에이전트 공통 컨텍스트
...
└── <id>.review.md   # draft plan review result
```
...
### 2. Reviewer plan gate + ID 기반 draft plan 승인

Planner가 `.opencode-test-agents/plans/<id>.plan.yaml`을 작성하면 orchestrator가 먼저 hidden `experiment-reviewer`를 호출합니다. Reviewer는 `.opencode-test-agents/plans/<id>.review.md`를 작성하고 `PASS`, `PASS_WITH_WARNINGS`, `BLOCKED` 중 하나를 반환합니다.

`BLOCKED`이면 planner가 수정해야 하므로 승인 gate로 넘어가지 않습니다. `PASS_WITH_WARNINGS`이면 경고를 확인한 뒤 명시 승인으로 진행할 수 있습니다. **명시적으로 "승인" 또는 "OK"라고 답하세요.** 모호한 응답("음... 좋아 보이네")은 승인으로 받아들이지 않습니다.
...
### 3. Executor 자동 handoff + reviewer script gate + 실행 승인
...
4. orchestrator가 `experiment-reviewer` script review를 실행해 `experiments/<id>/review.md` 기록
5. reviewer가 `PASS` 또는 `PASS_WITH_WARNINGS`일 때만 실행 직전 사용자에게 재확인
...
experiments/<id>/
├── plan.yaml
├── review.md               # generated scripts review result
├── env/{pre,post}/        # 환경 캡처
├── scripts/               # 실제 실행된 스크립트
...
| 항목 | Orchestrator | Planner | Reviewer | Executor |
|---|---|---|---|---|
| read / list / glob / grep | allow | allow | allow | allow |
| edit | deny | `.opencode-test-agents/plans/**`, `**/plan.yaml` allow | review artifacts만 allow | `experiments/**` allow, 그 외 ask |
| bash | read-only 일부 allow, 나머지 deny | git/ls/cat 등 read-only만 일부 allow, 나머지 deny | read-only 일부 allow, 나머지 deny | 환경 probe 명령 + `bash experiments/*` allow, 나머지 ask |
| webfetch | ask | ask | deny | deny |
| task (subagent 호출) | `experiment-planner`, `experiment-reviewer`, `experiment-executor` allow, 나머지 deny | deny | deny | deny |
...
새 Opencode 세션에서 이어갈 때는 `experiment-orchestrator`로 전환한 뒤 `.opencode-test-agents/plans/<id>.md`의 `Resume Prompt`를 붙여 넣으세요. Review 상태는 `.opencode-test-agents/plans/<id>.review.md`와 `experiments/<id>/review.md`에 남습니다. 수동 fallback할 때는 숨김 subagent 수동 호출이 가능한 환경에서 `@experiment-reviewer` 또는 `@experiment-executor`에 필요한 artifact 경로를 함께 전달하세요. 환경이 hidden subagent 수동 호출을 막으면 임시로 해당 agent의 `hidden`/`mode`를 바꿔 troubleshooting하고 끝나면 되돌립니다.
